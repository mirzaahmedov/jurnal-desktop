...
export class TabelService {
    ...
    static async getAll(ctx: QueryFunctionContext<[typeof TabelService.QueryKeys.GetAll, {
        page: number;
        limit: number;
        docNum?: string;
        year?: number;
        ...
        const { page, limit, docNum, year, month, budjetId } = ctx.queryKey[1]
    ...
    static async create(values: TabelFormValues) {
        const res = await zarplataApiNew.post<ZarplataApiResponse<Tabel>>(`${TabelService.endpoint}`, values)
        return res.data
    }

    static async updateChild(args: {
        id: number;
        values: TabelProvodkaFormValues
    }) {
        const res = await zarplataApiNew.put<ZarplataApiResponse<Tabel>>(`${TabelService.endpoint}/tabel-child/${args.id}`, args.values)
        return res.data
    }

    static async delete(id: number) {
        const res = await zarplataApiNew.delete<ZarplataApiResponse<void>>(`${TabelService.endpoint}/${id}`)
        return res.data
    }
}