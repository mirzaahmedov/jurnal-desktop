...
type ChooseOperatsiiProps = {
  selected?: Operatsii
  open: () => void
  clear: () => void
}
const ChooseOperatsii = (props: ChooseOperatsiiProps) => {
  const { selected, open, clear } = props

  const elements = [
    {
      name: 'Счет',
      value: selected?.schet
    },
    {
      name: 'Субсчет',
      value: selected?.sub_schet
    }
  ]
    ...
    <div className="border-2 border-slate-100 rounded-xl flex flex-wrap justify-between items-center gap-20">
      {selected ? (
        ...
        <div className="px-5 py-2.5 self-stretch flex items-center gap-10">
          <h6 className="text-sm text-slate-500">Выберите операцию</h6>
          <Button
            variant="ghost"
            size="icon"
            title="Выбрать"
...
export { ChooseOperatsii }
export type { ChooseOperatsiiProps }
