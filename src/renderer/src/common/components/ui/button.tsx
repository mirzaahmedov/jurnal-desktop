  ...
  'inline-flex gap-2 items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        outline:
          'border border-input bg-background shadow-none text-slate-500 hover:bg-accent hover:text-brand',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ children, className, variant, size, asChild = false, loading = false, ...props }, ref) => {
    ...
    return (
      ...
      >
        {children}
        {loading ? <Spinner className="text-white size-5 border-2" /> : null}
      </Comp>
    )
  }
)
